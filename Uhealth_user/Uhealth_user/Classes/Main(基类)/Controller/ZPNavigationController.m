...
- (void)moveViewWithX:(float)x
{
    ...
    x = MAX(MIN(x, kSCREEN_WIDTH), 0);
    // 设置frame的x
    self.view.frame = (CGRect){ {x, self.view.frame.origin.y}, self.view.frame.size};
    // 设置黑色蒙版的不透明度
    self.lastScreenBlackMask.alpha = 0.6 * (1 - (x / kSCREEN_WIDTH));
    // 设置上一个截屏的缩放比例
    CGFloat scale = x / kSCREEN_WIDTH * 0.05 + 0.95;
...
- (void)paningGestureReceive:(UIPanGestureRecognizer *)recoginzer
{
    ...
    if (UIGestureRecognizerStateBegan == recoginzer.state) {
        ...
        if (self.movingState == ZPNavMovingStateDragBegan || self.movingState == ZPNavMovingStateDragChanged) {
            ...
            [self moveViewWithX:[recoginzer translationInView:[[UIApplication sharedApplication] keyWindow]].x];
        }
    }
}
...
#define decelerationTime (0.4)
    // 获取手指离开时候的速率
    CGFloat velocityX = [panGestureRecognizer velocityInView:[[UIApplication sharedApplication] keyWindow]].x;
    // 手指拖拽的距离
    CGFloat translationX = [panGestureRecognizer translationInView:[[UIApplication sharedApplication] keyWindow]].x;
    // 按照一定decelerationTime的衰减时间，计算出来的目标位置
    CGFloat targetX = MIN(MAX(translationX + (velocityX * decelerationTime / 2), 0), kSCREEN_WIDTH);
    // 是否pop
    BOOL pop = ( targetX > ZPMinX );
    // 设置动画初始化速率为当前瘦子离开的速率
    CGFloat initialSpringVelocity = fabs(velocityX) / (pop ? kSCREEN_WIDTH - translationX : translationX);
    ...
    [UIView animateWithDuration:ZPAnimationDuration
         ...
         usingSpringWithDamping:1
          initialSpringVelocity:initialSpringVelocity
                     ...
                     animations:^{
                         [self moveViewWithX:pop ? kSCREEN_WIDTH : 0];
